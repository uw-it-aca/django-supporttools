...
![Django versions](https://img.shields.io/badge/django-4.2%20%7C%205.2-green.svg)


A Django application used for theming and wrapping support tools.

Supported Python versions are 3.10 and 3.12. CI tests each against Django 4.2
and 5.2.
...
# Required when this app defines SPA tools with a `vite_entry`.
# This is a filesystem path to the manifest generated by the app's Vite build,
# not a static URL. Update it for the collected-static location in production.
VITE_MANIFEST_PATH = BASE_DIR / 'myapp/static/.vite/manifest.json'
...
#   view           – dotted import path to the Django view class (required unless SUPPORTTOOLS_DEFAULT_SPA_VIEW is set)
SUPPORTTOOLS_VIEW_REGISTRY = [
  {
    "id": "retention-admin",
    "section": "application",
...
Enabling the shell does not convert any tool page; tools remain server-rendered
until they are configured as SPA entries in Phase 4.
...
registered here, a simple `custom_sidebar_links.html` override can be removed.
Keep an override that supplies custom sections, conditional links, or required
ordering until the rendered Vue navigation has been verified to preserve those
requirements.
...
**2. Add the default view and register SPA URLs**:
...
If your project does not already include `supporttools.urls`, append generated
SPA routes after your application routes:
...
Do not add a root-level `include("supporttools.urls")` solely to enable SPA
routes: its legacy home route is a catch-all. Alternatively, keep an existing
project URL and change that URL's view to your authenticated SPA view. In either
case, each registry `route` must resolve to the URL served by that view.
...
Each `vite_entry` must be an input in this app-owned Vite build. Build the
assets with `npm run build` before `collectstatic`, and configure
...
The URL is registered by the Phase 3 URL handoff. No additional `urls.py` edit
